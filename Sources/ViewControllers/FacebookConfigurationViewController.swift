//
//  FacebookConfigurationViewController.swift
//  Example
//
//  Created by Brommko LLC on 31/12/2018.
...
class FacebookConfigurationViewController: FormViewController {
    ...
    func createDefaultsSection() -> SectionFormer {
        let productId = TextFieldRowFormer<FormTextFieldCell>() {
            $0.titleLabel.font = .boldSystemFont(ofSize: 15)
                ...
                $0.placeholder = "Your facebook id"
                $0.text = SuperView.shared.options.facebook.facebookID
            }.onTextChanged { text in
                SuperView.shared.options.facebook.facebookID = text
        }
        return SectionFormer(rowFormer: productId).set(headerViewFormer: self.createHeader("Facebook ID"))
    }
    ...
    private func configure() {
        ...
        let isEnabled = SwitchRowFormer<FormSwitchCell>() {
            ...
            $0.titleLabel.font = .boldSystemFont(ofSize: 15)
                ...
                $0.switched = SuperView.shared.options.facebook.isEnabled
            }.onSwitchChanged { on in
                SuperView.shared.options.facebook.isEnabled = on
                self.switchDefaultsSection()
        }

        // Create SectionFormers
        let isEnabledSection = SectionFormer(rowFormer: isEnabled).set(headerViewFormer: self.createHeader("Facebook"))
        self.former.removeAll().append(sectionFormer: isEnabledSection).reload()
        if SuperView.shared.options.facebook.isEnabled {
